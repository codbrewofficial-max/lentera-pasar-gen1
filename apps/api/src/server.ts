...
import { AppError, ok, publicUser, toErrorPayload } from "./http.js";
  ...
  return {
    ...
    page: {
      ...
      sections: page.sections.map((section: any) => ({
        ...
        content: section.contentJson || section.templateSection?.defaultContentJson || {},
...
const recordTracking = async (request: FastifyRequest, input: z.infer<typeof trackingBody>) => {
  ...
  if (!website) throw new AppError(404, "TRACKING_KEY_NOT_FOUND", "Tracking key not found");
...
const registerAuthRoutes = () => {
  app.post("/api/v1/auth/register", async (request, reply) => {
    ...
    return ok(reply, { user: publicUser(user), token: app.jwt.sign({ id: user.id, role: user.role, email: user.email }) }, "Registered", 201);
  });
...
const registerInternalRoutes = () => {
  ...
  app.post("/api/v1/internal/owners", async (request, reply) => {
    ...
    return ok(reply, publicUser(owner), "Owner created", 201);
  });
...
const registerWebsiteRoutes = () => {
  app.get("/api/v1/websites", async (request, reply) => {
    ...
    return ok(reply, websites, "Websites loaded");
  });
  ...
  app.patch("/api/v1/websites/:websiteId", async (request: Req, reply) => {
    ...
    return ok(reply, updated, "Website updated");
  });
  app.post("/api/v1/websites/:websiteId/publish", async (request: Req, reply) => {
    ...
    return ok(reply, updated, "Website published");
  });
  app.post("/api/v1/websites/:websiteId/unpublish", async (request: Req, reply) => {
    ...
    return ok(reply, updated, "Website unpublished");
  });
  app.get("/api/v1/websites/:websiteId/pages", async (request: Req, reply) => {
    const { website } = await getWebsiteForAccess(request);
    const pages = await prisma.websitePage.findMany({ where: { websiteId: website.id }, orderBy: { sortOrder: "asc" } });
    return ok(reply, pages, "Pages loaded");
  });
  app.get("/api/v1/websites/:websiteId/pages/:pageKey", async (request: Req, reply) => {
    const { website } = await getWebsiteForAccess(request);
    const page = await prisma.websitePage.findUnique({ where: { websiteId_pageKey: { websiteId: website.id, pageKey: request.params?.pageKey || "" } }, include: { sections: true } });
    if (!page) throw new AppError(404, "PAGE_NOT_FOUND", "Page not found");
    return ok(reply, page, "Page loaded");
  });
};
...
const registerSectionRoutes = () => {
  app.get("/api/v1/websites/:websiteId/sections", async (request: Req, reply) => {
    ...
    return ok(reply, sections, "Sections loaded");
  });
  app.get("/api/v1/websites/:websiteId/sections/:slotKey", async (request: Req, reply) => {
    ...
    return ok(reply, section, "Section loaded");
  });
  app.patch("/api/v1/websites/:websiteId/sections/:slotKey/template", async (request: Req, reply) => {
    ...
    const updated = await prisma.pageSection.update({ where: { id: section.id }, data: { templateSectionId: template.id } });
    return ok(reply, updated, "Section template updated");
  });
  app.patch("/api/v1/websites/:websiteId/sections/:slotKey/content", async (request: Req, reply) => {
    ...
    return ok(reply, updated, "Section content updated");
  });
  app.patch("/api/v1/websites/:websiteId/sections/:slotKey/visibility", async (request: Req, reply) => {
    ...
    return ok(reply, updated, "Section visibility updated");
  });
};
...
const registerTemplateRoutes = () => {
  app.post("/api/v1/internal/template-sections/import-zip", async (request, reply) => {
      ...
      imported.push(
        await prisma.templateSection.upsert({
          ...
          update: {
            ...
            schemaJson: parsed.schema as any,
    ...
    return ok(reply, { importedCount: imported.length, sections: imported }, "Template sections imported");
  });
  app.get("/api/v1/template-sections", async (request: Req, reply) => {
    ...
    return ok(reply, sections, "Template sections loaded");
  });
  app.get("/api/v1/template-sections/:id", async (request: Req, reply) => {
    ...
    return ok(reply, section, "Template section loaded");
  });
...
const registerPublicRoutes = () => {
  app.get("/api/v1/public/sites/:slug", async (request: Req, reply) => {
    ...
    if (!website) throw new AppError(404, "PUBLIC_SITE_NOT_FOUND", "Published site not found");
  ...
  app.get("/api/v1/websites/:websiteId/preview/pages/:pageKey", async (request: Req, reply) => {
    const { website } = await getWebsiteForAccess(request);
    return ok(reply, await buildPublicPage(website.id, { pageKey: request.params?.pageKey || "home" }), "Preview page loaded");
  });
  app.post("/api/v1/public/tracking/events", { config: { rateLimit: { max: 60, timeWindow: "1 minute" } } }, async (request, reply) => {
    ...
    try {
      ...
      request.log.warn(error, "tracking failed");
      return ok(reply, { accepted: false }, "Tracking event ignored", 202);
    }
  });
  app.post("/api/v1/public/sites/:slug/contact", { config: { rateLimit: { max: 20, timeWindow: "1 minute" } } }, async (request: Req, reply) => {
    ...
    return ok(reply, lead, "Lead created", 201);
  });
};
...
const registerLeadAndInsightRoutes = () => {
  app.get("/api/v1/websites/:websiteId/leads", async (request: Req, reply) => {
    const { website } = await getWebsiteForAccess(request);
    return ok(reply, await prisma.lead.findMany({ where: { websiteId: website.id }, orderBy: { createdAt: "desc" } }), "Leads loaded");
  });
  app.get("/api/v1/websites/:websiteId/leads/recent", async (request: Req, reply) => {
    const { website } = await getWebsiteForAccess(request);
    return ok(reply, await prisma.lead.findMany({ where: { websiteId: website.id }, orderBy: { createdAt: "desc" }, take: 10 }), "Recent leads loaded");
  });
  app.patch("/api/v1/websites/:websiteId/leads/:leadId/status", async (request: Req, reply) => {
    ...
    return ok(reply, await prisma.lead.update({ where: { id: lead.id }, data: { status: body.status } }), "Lead status updated");
  });
  ...
  app.get("/api/v1/websites/:websiteId/insights/summary", async (request: Req, reply) => {
    ...
    return ok(reply, {
      ...
      totalWhatsappClicks,
      totalContactSubmits,
      topPage: topPageRows[0] ? { pageKey: topPageRows[0].pageKey, total: topPageRows[0].total } : null,
      topService: topService ? { objectId: topService.id, title: topService.title, total: topServiceRows[0].total } : null,
      topPortfolio: topPortfolio ? { objectId: topPortfolio.id, title: topPortfolio.title, total: topPortfolioRows[0].total } : null
    }, "Insight summary loaded");
  });
  app.get("/api/v1/websites/:websiteId/insights/top-pages", async (request: Req, reply) => ok(reply, await topBy((await getWebsiteForAccess(request)).website.id, "pageKey", { eventName: "page_view" }), "Top pages loaded"));
  app.get("/api/v1/websites/:websiteId/insights/top-sections", async (request: Req, reply) => ok(reply, await topBy((await getWebsiteForAccess(request)).website.id, "slotKey", { eventName: { in: ["section_view", "cta_click"] } }), "Top sections loaded"));
  app.get("/api/v1/websites/:websiteId/insights/top-ctas", async (request: Req, reply) => ok(reply, await topBy((await getWebsiteForAccess(request)).website.id, "ctaKey", { eventName: { in: ["cta_click", "whatsapp_click"] } }), "Top CTAs loaded"));
  app.get("/api/v1/websites/:websiteId/insights/top-services", async (request: Req, reply) => ok(reply, await topBy((await getWebsiteForAccess(request)).website.id, "objectId", { eventName: "service_view", objectType: "service" }), "Top services loaded"));
  app.get("/api/v1/websites/:websiteId/insights/top-portfolios", async (request: Req, reply) => ok(reply, await topBy((await getWebsiteForAccess(request)).website.id, "objectId", { eventName: "portfolio_view", objectType: "portfolio" }), "Top portfolios loaded"));
  app.get("/api/v1/websites/:websiteId/insights/traffic-sources", async (request: Req, reply) => {
    ...
    return ok(reply, Array.from(sources.entries()).map(([source, total]) => ({ source, total })).sort((a, b) => b.total - a.total), "Traffic sources loaded");
  });
};
