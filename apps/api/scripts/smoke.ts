...
async function main() {
  ...
  const initialWebsiteList = await request("/api/v1/websites", { headers: auth(ownerToken) });
  ...
  const pages = await request(`/api/v1/websites/${websiteId}/pages`, { headers: auth(ownerToken) });
  assert(pages.data.length === 6, `Expected 6 default pages, got ${pages.data.length}`);
  ...
  const homePage = await request(`/api/v1/websites/${websiteId}/pages/home`, { headers: auth(ownerToken) });
  assert(homePage.data.sections[0].slotLabel, "Page detail section missing slotLabel");

  const sections = await request(`/api/v1/websites/${websiteId}/sections`, { headers: auth(ownerToken) });
  assert(sections.data.length === 14, `Expected 14 default sections, got ${sections.data.length}`);
  const initialHeroSection = await request(`/api/v1/websites/${websiteId}/sections/home.hero`, { headers: auth(ownerToken) });
  ...
  const templates = await request("/api/v1/template-sections/by-slot/home.hero", { headers: auth(ownerToken) });
  ...
  await request(`/api/v1/template-sections/${templates.data[0].id}`, { headers: auth(ownerToken) });
  await request("/api/v1/template-sections?websiteType=company_profile&slotKey=home.hero", { headers: auth(ownerToken) });
  ...
  await request(`/api/v1/websites/${websiteId}/business-profile`, { headers: auth(ownerToken) });
  ...
  const preview = await request(`/api/v1/websites/${websiteId}/preview/pages/home`, { headers: auth(ownerToken) });
  ...
  const recentLeads = await request(`/api/v1/websites/${websiteId}/leads/recent`, { headers: auth(ownerToken) });
  ...
  await request(`/api/v1/websites/${websiteId}/leads`, { headers: auth(ownerToken) });
  ...
  const summary = await request(`/api/v1/websites/${websiteId}/insights/summary`, { headers: auth(ownerToken) });
  ...
  const topPages = await request(`/api/v1/websites/${websiteId}/insights/top-pages`, { headers: auth(ownerToken) });
  assert(topPages.data[0].pageLabel, "Top pages missing pageLabel");
  const topSections = await request(`/api/v1/websites/${websiteId}/insights/top-sections`, { headers: auth(ownerToken) });
  assert(topSections.data[0].slotLabel, "Top sections missing slotLabel");
  const topCtas = await request(`/api/v1/websites/${websiteId}/insights/top-ctas`, { headers: auth(ownerToken) });
  assert(topCtas.data[0].ctaLabel, "Top CTAs missing ctaLabel");
  const topServices = await request(`/api/v1/websites/${websiteId}/insights/top-services`, { headers: auth(ownerToken) });
  assert(topServices.data[0].serviceId, "Top services missing serviceId");
  const topPortfolios = await request(`/api/v1/websites/${websiteId}/insights/top-portfolios`, { headers: auth(ownerToken) });
  assert(topPortfolios.data[0].portfolioId, "Top portfolios missing portfolioId");
  const trafficSources = await request(`/api/v1/websites/${websiteId}/insights/traffic-sources`, { headers: auth(ownerToken) });
