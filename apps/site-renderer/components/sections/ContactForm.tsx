...
export function ContactForm({ siteSlug, pageKey, slotKey = 'contact.contact_cta' }: Props) {
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    // Di beberapa kondisi React/browser, event.currentTarget bisa null setelah await.
  ...
  return (
    <form onSubmit={onSubmit} className="lp-card grid gap-4 p-5 md:p-6">
      ...
      <div className="grid gap-2">
        <label className="text-sm font-bold text-slate-700">Minat / Kebutuhan</label>
        <input
          name="interest"
          className="rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-blue-500"
      ...
      <button
        ...
        className="lp-btn lp-btn-primary w-full disabled:cursor-not-allowed disabled:opacity-60"
      >
