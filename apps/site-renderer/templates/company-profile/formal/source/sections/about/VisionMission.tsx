  ...
  return (
    <section id="about-vision-mission" className="py-16 md:py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className={`grid grid-cols-1 ${imageUrl ? "lg:grid-cols-12" : ""} gap-8 items-stretch`}>
          {imageUrl && (
            <div className="lg:col-span-4 relative">
              <div className="relative rounded-none overflow-hidden shadow-xl border border-slate-100 bg-slate-50 p-2 h-full min-h-[280px]">
                <img src={imageUrl} alt="" className="w-full h-full object-cover rounded-none absolute inset-0 p-2" referrerPolicy="no-referrer" />
                <div className="absolute inset-0 bg-gradient-to-t from-slate-950/20 via-transparent to-transparent" />
              </div>
            </div>
          )}

          <div className={`grid grid-cols-1 sm:grid-cols-2 gap-8 ${imageUrl ? "lg:col-span-8" : ""}`}>
