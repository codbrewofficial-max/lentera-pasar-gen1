          ...
          {filtered.map((item) => (
            <Card key={item.slug} className="flex flex-col h-full bg-white border border-slate-100 shadow-sm">
              ...
              <div className="p-5 flex flex-col flex-grow">
                ...
                <h4 className="text-sm md:text-base font-semibold text-slate-900 mb-2 leading-snug tracking-tight hover:text-[#1E3A5F] transition-colors line-clamp-2">
                ...
                <Button href={`${baseHref}/${item.slug}`} variant="text" size="sm" iconRight={<ArrowRight className="w-3.5 h-3.5 ml-1" />} className="p-0 text-xs font-semibold text-[#1E3A5F] mt-auto self-start">Selengkapnya</Button>
              </div>
            </Card>
          ))}
        </div>
