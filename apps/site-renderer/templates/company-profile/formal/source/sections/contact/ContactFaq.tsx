...
export const ContactFaq: React.FC<ContactFaqProps> = ({
  ...
  return (
    <section id="contact-faq-form" className="bg-slate-50">
      {imageUrl ? (
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 md:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 items-start">
            ...
            <Card className="p-6 md:p-8 bg-white border border-slate-100 shadow-lg">
              {status === "success" ? (
                <div className="text-center py-8">
                  <div className="mx-auto w-14 h-14 bg-emerald-50 text-emerald-600 rounded-none flex items-center justify-center mb-4"><CheckCircle className="w-7 h-7" /></div>
                ...
                <form className="space-y-5" onSubmit={handleSubmit}>
                  <div className="grid grid-cols-1 sm:grid-cols-1 gap-5">
                    <div>
                      ...
                      <input name="name" required type="text" className="w-full rounded-none border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#1E3A5F]" placeholder="Nama Anda" />
                    </div>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-2 font-mono">Email</label>
                      <input name="email" type="email" className="w-full rounded-none border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#1E3A5F]" placeholder="email@contoh.com" />
                    </div>
                    <div>
                      ...
                      <input name="phone" type="tel" className="w-full rounded-none border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#1E3A5F]" placeholder="08xxxxxxxxxx" />
                    </div>
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-2 font-mono">Kebutuhan</label>
                    <input name="interest" className="w-full rounded-none border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#1E3A5F]" placeholder="Contoh: konsultasi layanan" />
                  </div>
                  <div>
                    <label className="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-2 font-mono">Pesan</label>
                    <textarea name="message" required rows={5} className="w-full rounded-none border border-slate-200 px-4 py-3 text-sm outline-none focus:border-[#1E3A5F]" placeholder="Ceritakan kebutuhan Anda secara singkat." />
                  </div>
                  ...
                  {message && status === "error" && <p className="rounded-none border border-red-100 bg-red-50 px-4 py-3 text-sm text-red-700">{message}</p>}
          ...
          <div className="lg:col-span-5 flex flex-col space-y-6">
            <div className="border border-slate-200 rounded-none p-6 bg-slate-50/20">
              <h3 className="text-sm font-semibold uppercase tracking-wider text-slate-900 mb-2 font-mono flex items-center space-x-1.5"><HelpCircle className="w-4.5 h-4.5 text-[#8A6D3B]" /><span>Bantuan Cepat</span></h3>
