...
export function FormalArticleDetailHero(props: FormalSectionProps) {
  ...
  return (
    <FormalSection muted>
      <div className="mx-auto max-w-4xl text-center">
        <div className="flex justify-center gap-2">
          {article.category?.name && (
            <span className="inline-flex rounded-full bg-blue-50 px-3 py-1 text-xs font-bold uppercase tracking-[0.16em] text-blue-700 ring-1 ring-blue-100">
              {article.category.name}
            </span>
          )}
        </div>
        ...
        <p className="mt-5 text-lg leading-8 text-slate-600">{descriptionOf(article)}</p>
      ...
      {showCover && imageOf(article) && (
        <div className="mx-auto mt-10 max-w-5xl">
          <FormalImageFrame imageUrl={imageOf(article)} alt={titleOf(article)} label="Article Cover" />
        </div>
      )}
    </FormalSection>
  );
...
// Schema: contentMaxWidth, showShareHint
export function FormalArticleContent(props: FormalSectionProps) {
  const c = props.section.content || {};
  const article = props.section.data?.article || {};
  const showShare = boolValue(c.showShareHint, true);
  const paragraphs = String(article.content || article.description || article.excerpt || '')
    .split(/\n{2,}/)
    .filter(Boolean);
  return (
    <FormalSection>
      <article className={`mx-auto ${maxWidthClass(c.contentMaxWidth)} text-slate-700`}>
        {paragraphs.length ? (
          paragraphs.map((paragraph, index) => (
            <p key={index} className="mt-6 first:mt-0 text-lg leading-9">{paragraph}</p>
          ))
        ) : (
          <FormalEmpty title="Konten artikel belum tersedia" description="Isi konten artikel di dashboard agar detail artikel tampil lengkap." />
        )}
        {showShare && (
          <div className="mt-10 rounded-2xl border border-slate-200 bg-slate-50 p-5">
            <p className="text-sm font-semibold text-slate-950">Bagikan artikel ini</p>
            <p className="mt-2 text-sm leading-6 text-slate-600">
              Salin tautan halaman ini dari browser untuk membagikan artikel kepada calon klien atau rekan kerja.
            </p>
          </div>
        )}
      </article>
  ...
  return (
    <FormalSection muted>
      ...
      {items.length ? (
            ...
            <a key={item.id || item.slug || index} href={getArticleHref(props.siteSlug, props.payload, item.slug)} className="block h-full">
              <FormalCard className="h-full p-6 transition hover:-translate-y-1 hover:border-[#649FF6]/40 hover:shadow-md">
                <div className="mb-3 flex flex-wrap gap-2">
                  {item.category?.name && (
                    <span className="inline-flex rounded-full bg-blue-50 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.16em] text-blue-700 ring-1 ring-blue-100">
                      {item.category.name}
                    </span>
                  )}
                </div>
        ...
        <div className="mt-8">
          <FormalEmpty title="Belum ada artikel terkait" description="Artikel terkait akan tampil ketika ada artikel published lain." />
        </div>
      )}
    </FormalSection>
  );
      ...
      <div className="rounded-3xl bg-slate-950 p-8 text-white md:p-12">
        <div className="grid gap-8 md:grid-cols-[1fr_auto] md:items-center">
          <FormalHeading
            ...
            title={text(c.title, 'Butuh Bantuan Setelah Membaca Artikel Ini?')}
            description={text(c.description, 'Hubungi tim kami untuk mendiskusikan kebutuhan Anda secara lebih terarah.')}
          />
