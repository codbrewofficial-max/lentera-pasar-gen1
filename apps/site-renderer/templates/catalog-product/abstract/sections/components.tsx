...
function OptionalCta({ props, fallbackPath }: { props: AbstractCatalogProductSectionProps; fallbackPath: string }) {
  ...
  return (
    <div className="mt-6">
      <AbstractButton href={ctaHref} label={ctaLabel} />
    </div>
  );
}

// Header ringkas & opsional — HANYA render kalau salah satu field diisi owner.
// `light` menyesuaikan warna teks dengan background section (dark section pakai light=true).
function OptionalIntro({ props, center = false, light = false }: { props: AbstractCatalogProductSectionProps; center?: boolean; light?: boolean }) {
  const content = contentOf(props.section);
  ...
  return (
    <div className={center ? "mx-auto mb-8 max-w-2xl text-center" : "mb-8 max-w-2xl"}>
      {badge && <AbstractBadge label={badge} light={light} />}
      {title && <h2 className={`mt-3 text-2xl font-extrabold tracking-tight ${light ? "text-white" : "text-neutral-900"}`}>{title}</h2>}
      {subtitle && <p className={`mt-2 text-sm leading-7 ${light ? "text-neutral-300" : "text-neutral-600"}`}>{subtitle}</p>}
    </div>
  );
}
...
export function AbstractCatalogHomeHero(props: AbstractCatalogProductSectionProps) {
  ...
  const description = pick(content.subtitle, activeBanner?.subtitle, business.description, "Produk pilihan dengan tampilan yang segar dan pengalaman belanja yang menyenangkan.");
...
export function AbstractCatalogHomeCategoryShowcase(props: AbstractCatalogProductSectionProps) {
  ...
  const description = text(content.subtitle, "Temukan produk berdasarkan kategori kesukaanmu.");
...
export function AbstractCatalogHomeFeaturedProducts(props: AbstractCatalogProductSectionProps) {
  ...
  const description = text(content.subtitle, "Favorit pelanggan yang paling sering dipilih.");
    ...
    <section className="lp-section bg-[#151515] text-white">
      <div className="lp-container">
        <div className="flex flex-wrap items-end justify-between gap-6">
          ...
          <a href={sectionHref(props, "cta", "/products")} className="inline-flex items-center gap-2 text-sm font-bold text-[#649FF6] hover:underline">{text(content.ctaLabel, "Lihat Semua")} <ArrowRight className="h-4 w-4" /></a>
        </div>
        {list.length ? (
...
export function AbstractCatalogHomeBrandTrust(props: AbstractCatalogProductSectionProps) {
  ...
  const description = pick(content.subtitle, business.description, "Kami senang bisa bantu kebutuhan belanjamu dengan produk berkualitas dan tampilan yang segar.");
  ...
  return (
    <section className="border-b border-neutral-100 bg-neutral-50 py-8">
      <div className="lp-container">
        ...
        {badge && <AbstractBadge label={badge} />}
...
export function AbstractCatalogProductDetailRecommendation(props: AbstractCatalogProductSectionProps) {
  ...
  return (
    <section className="lp-section bg-white">
      <div className="lp-container">
        {badge && <AbstractBadge label={badge} />}
        <h2 className="mt-3 text-2xl font-extrabold tracking-tight text-neutral-900">{title}</h2>
...
export function AbstractCatalogFaqHeroSearch(props: AbstractCatalogProductSectionProps) {
  ...
  return (
    <section className="relative overflow-hidden bg-[#151515] py-16 text-white md:py-20">
      {backgroundImage && (
        <>
          <img src={backgroundImage} alt="" className="absolute inset-0 h-full w-full object-cover opacity-25" referrerPolicy="no-referrer" />
          <div className="absolute inset-0 bg-gradient-to-b from-[#151515]/70 to-[#151515]" />
        </>
      )}
...
export function AbstractCatalogFaqContactCta(props: AbstractCatalogProductSectionProps) {
  ...
  const description = text(content.subtitle, "Tim kami siap bantu jawab pertanyaanmu.");
  ...
  return (
    <section className="relative overflow-hidden border-t border-neutral-100 bg-neutral-50 py-16">
      {backgroundImage && (
        <>
          <img src={backgroundImage} alt="" className="absolute inset-0 h-full w-full object-cover opacity-10" referrerPolicy="no-referrer" />
          <div className="absolute inset-0 bg-gradient-to-b from-neutral-50/60 to-neutral-50" />
        </>
      )}
      <div className="relative lp-container text-center">
        {badge && <AbstractBadge label={badge} />}
        <h2 className="mt-3 text-2xl font-extrabold tracking-tight text-neutral-900 md:text-3xl">{title}</h2>
...
export function AbstractCatalogArticleDetailHeaderMeta(props: AbstractCatalogProductSectionProps) {
  ...
  return (
    <section className="relative overflow-hidden border-b border-neutral-100 bg-neutral-50 py-12">
      {backgroundImage && (
        <>
          <img src={backgroundImage} alt="" className="absolute inset-0 h-full w-full object-cover opacity-10" referrerPolicy="no-referrer" />
          <div className="absolute inset-0 bg-gradient-to-b from-neutral-50/60 to-neutral-50" />
      ...
      <div className="relative lp-container max-w-3xl">
        {badge ? <AbstractBadge label={badge} /> : article.category?.name && <p className="font-mono text-xs lowercase tracking-wide text-[#649FF6]">{article.category.name}</p>}
...
export function AbstractCatalogArticleDetailProductCta(props: AbstractCatalogProductSectionProps) {
  ...
  return (
    <section className="lp-section bg-neutral-50">
      <div className="lp-container">
        {badge && <AbstractBadge label={badge} />}
        <h2 className="mt-3 text-xl font-extrabold text-neutral-900">{title}</h2>
        {subtitle && <p className="mt-2 text-sm text-neutral-600">{subtitle}</p>}
...
export function AbstractCatalogArticleDetailComments(props: AbstractCatalogProductSectionProps) {
  return (
...
export function AbstractCatalogContactInquiryForm(props: AbstractCatalogProductSectionProps) {
  ...
  const description = text(content.subtitle, "Isi formulir berikut, tim kami akan menghubungimu dengan penawaran terbaik.");
